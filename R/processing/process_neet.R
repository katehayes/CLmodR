...
neet_gender_pc_c <- neet_16to23_age_gender %>% 
  ...
  full_join(neet_11to23_age %>% 
              ...
              mutate(gender = "All")) %>% 
  add_cohort() %>% 
  filter(!cohort %in% c("Cohort 2", "Cohort 8", "Cohort 9", "Cohort 15")) %>% 
  ggplot() +
...
neet_gender_pc_c
ggsave(filename = "output/graphs/neet_gender_pc_c.png", neet_gender_pc_c)
