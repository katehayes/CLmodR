...
fte_data <- read_xlsx("/Users/katehayes/Library/CloudStorage/GoogleDrive-khayes2@sheffield.ac.uk/My Drive/CL_drive_data/fte_age_ew (1).xlsx")
...
fte2020 <- fte_nc_cust_age %>% 
  filter(t == 520) 

fte_nc_cust_age <- fte_nc_cust_age %>% 
  filter(t != 0) %>% 
  mutate(t = t - 52) %>% 
  rbind(fte2020)
...
# TAKEN FROM ELSEWHERE (THE WHAT IS THIS FILE) AND DEPOSITED HERE
